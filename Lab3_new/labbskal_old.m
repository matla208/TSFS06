...
%% --- c5 ---
% Sätt parametrar för konsistensrelation
%c5params.x0   = -h2Init; % Initialvärde för residualgeneratorns tillstånd
%c5params.alfa = 2;       % Placering av polen i -alfa
%c5params.d    = d;       % Modellparametrar

%% --- c6 ---
% Sätt parametrar för konsistensrelation
%c6params.x0   = -h2Init; % Initialvärde för residualgeneratorns tillstånd
%c6params.alfa = 2;       % Placering av polen i -alfa
...
%%
 Jnorm=ones(1,4); % Default är alla trösklar satta till 1

%% standard diviation assumtion gives: 
sigma = std(res);
mu = mean(res);

Jnorm = norminv((1-pfa/2),mu,sigma);
%%
Jnorm = chi2inv((1-pfa),length(res)-1);
